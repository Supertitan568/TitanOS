...
#define VGA_ERROR(message) ERROR("VGA", message)
...
static void scroll_down(){
  ...
  memset((void*)(vga_text_buffer + (VGA_LENGTH - line_length)), 0, line_length);
...
bool set_cursor_buffer(vga* new_buff){
  // TODO: Check to make sure buffer is valid
  cursor.vga_out = new_buff;
  return true;
}

void console_init(){
  // Resets console
  set_cursor_buffer((vga*) VGA_TEXT_BUFFER_BASE);
  clear_console();
  cursor.x = 0;
  cursor.y = 0;
  move_cursor();
...
static void printint(int d){ 
  ...
  for(int i = sizeof(num_str) - 1; i > 0; i--){
    digit = d % 10;
    num_str[i - 1] = 48 + digit;
    d = (d - digit) / 10;
    cut_num_str = num_str + i;
    if(d == 0){break;}
  }
  
  printstr(cut_num_str);
}
    ...
    if(*current_char == '%'){
      switch(*(current_char + 1)){
        ...
        case 'l':
          ...
          break;
        default:
          VGA_ERROR("Format Specifier not found")
          current_char += 2;
      }
      continue;
    }
